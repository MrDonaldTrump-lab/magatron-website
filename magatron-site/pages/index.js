...
// If you're on Next.js, uncomment the next line and wrap the page with <Head> below
// import Head from "next/head";

/**
 * HOW TO USE
 * 1) Place /public/icons/magatron-32.png and /public/icons/magatron-512.png (your red spiked emblem)
 * 2) If using Next.js, uncomment Head import and the <Head> block for favicon + social previews
 * 3) Replace placeholder links for X (Twitter), Discord, contract address, and presale URL
 */
...
export default function Home() {
  // Ensure favicon shows the M logo across stacks (Next.js/CRA/Vite)
  useEffect(() => {
    const ensureFavicon = (href) => {
      let link = document.querySelector("link[rel='icon'][type='image/png']") || document.querySelector("link[rel='icon']");
      if (!link) {
    ...
    ensureFavicon('/icons/magatron-32.png');
  }, []);
  return (
    <div className="relative min-h-screen text-white font-sans bg-gradient-to-br from-red-950 via-black to-red-900 overflow-hidden">
      {/* ---- Favicons & Social Preview (Next.js only) ---- */}
      {false && (
        <Head>
          <title>MAGATron ($MAGT)</title>
          <meta name="description" content="The intergalactic memecoin. To the moon… and beyond tariffs." />
          ...
          <meta property="og:description" content="The intergalactic memecoin. To the moon… and beyond tariffs." />
          <meta property="og:image" content="/icons/magatron-512.png" />
          <meta name="twitter:card" content="summary_large_image" />
          <meta name="twitter:title" content="MAGATron ($MAGT)" />
          <meta name="twitter:description" content="The intergalactic memecoin. To the moon… and beyond tariffs." />
      ...
      <header className="relative z-10 border-b border-red-800/50 bg-black/40 backdrop-blur-xl">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href=\"https://x.com/MAGATron_Coin\" target="_blank" rel="noreferrer" className="px-4 py-2 rounded-xl bg-white text-black font-semibold hover:bg-red-300">Follow X</a>
            <a href="/presale" className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-500 font-semibold">Presale</a>
          </div>
        </div>
      </header>
      ...
      <section className=\"relative z-10 text-center py-14 md:py-20 bg-gradient-to-r from-red-800/40 to-red-600/20\">
        <img src=\"/magatron-logo.png\" alt=\"MAGATron Logo\" className=\"mx-auto w-40 md:w-48 mb-5 drop-shadow-[0_10px_30px_rgba(255,0,0,0.35)]\" />
        <h1 className=\"text-4xl md:text-6xl font-extrabold tracking-tight\">MAGATron</h1>
        <p className=\"text-lg md:text-xl mt-3 text-red-100\">The intergalactic memecoin — to the moon… and beyond tariffs.</p>
        <div className=\"mt-6 flex justify-center gap-3\">
          <a className=\"bg-white text-black font-bold py-2.5 px-5 rounded-2xl hover:bg-red-300\" href=\"https://x.com/MAGATron_Coin\" target=\"_blank\" rel=\"noreferrer\">Follow X</a>
          <a className=\"bg-white/10 text-white font-bold py-2.5 px-5 rounded-2xl hover:bg-white/20\" href=\"#tokenomics\">View Tokenomics</a>
        </div>
      ...
      <motion.section variants={fadeIn} initial=\"hidden\" whileInView=\"show\" viewport={{ once: true }} className=\"relative z-10 flex flex-col items-center py-12 px-6 bg-gradient-to-t from-black/90 via-red-950/80 to-black\">
      ...
      {/* About */}
      <motion.section id="about" variants={fadeIn} initial="hidden" whileInView="show" viewport={{ once: true }} className="relative z-10 px-6 py-12 max-w-5xl mx-auto bg-black/50 backdrop-blur-md rounded-2xl border border-red-700">
        <h2 className="text-3xl font-bold mb-4 text-red-100">About MAGATron</h2>
        <p className="text-white/90 leading-relaxed">
          MAGATron is a meme-fueled, community-powered ERC‑20 built for speed and fun. Fixed supply, no minting after deploy, and
          clear launch protection via trading gates and anti‑whale limits. We grow through memes, contests, and transparent updates.
        </p>
      </motion.section>

      {/* Tokenomics */}
      <motion.section id="tokenomics" variants={fadeIn} initial="hidden" whileInView="show" viewport={{ once: true }} className="relative z-10 px-6 py-12 max-w-6xl mx-auto">
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <InfoCard title="Links">
            <ul className="list-disc list-inside text-white/90 space-y-1">
              <li>Contract: <span className="text-white/70">0x…youraddress</span></li>
              <li><a href="/whitepaper.pdf" className="underline hover:text-white">Whitepaper</a></li>
              <li><a href=\"https://x.com/MAGATron_Coin\" target="_blank" rel="noreferrer" className="underline hover:text-white">X (Twitter)</a></li>
              <li><a href="/discord" className="underline hover:text-white">Discord</a></li>
            </ul>
          </InfoCard>
        </div>
      </motion.section>
      ...
      {/* Roadmap */}
      <motion.section id="roadmap" variants={fadeIn} initial="hidden" whileInView="show" viewport={{ once: true }} className="relative z-10 px-6 py-12 max-w-6xl mx-auto">
      ...
      {/* Whitepaper */}
      <motion.section id="whitepaper" variants={fadeIn} initial="hidden" whileInView="show" viewport={{ once: true }} className="relative z-10 px-6 py-12 max-w-5xl mx-auto">
        <div className="backdrop-blur-md bg-black/70 p-6 rounded-2xl border border-white/10 relative">
          ...
          <p className="text-white/90 mb-5">Dive into mission, token design, launch protection, and community growth mechanics.</p>
          <a href="/whitepaper.pdf" className="inline-block border border-white text-white py-2.5 px-5 rounded-xl hover:bg-white hover:text-black">View Full Whitepaper</a>
        </div>
      </motion.section>
      ...
      <motion.section variants={fadeIn} initial="hidden" whileInView="show" viewport={{ once: true }} className="relative z-10 bg-gradient-to-r from-red-700 to-black px-6 py-12 text-center border-t border-red-800">
        ...
        <div className="flex justify-center gap-4">
          <a href=\"https://x.com/MAGATron_Coin\" target="_blank" rel="noreferrer" className="border border-white text-white py-2 px-4 rounded hover:bg-white hover:text-black">Twitter</a>
        </div>
      </motion.section>
      ...
      {/* Utilities */}
      <style>{`
        .badge{ @apply px-3 py-1 rounded-full bg-white/10 text-white/80 border border-white/10; }
      `}</style>
    </div>
  );
}
